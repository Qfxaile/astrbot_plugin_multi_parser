...
| 腾讯频道 | 帖子内视频 | 帖子正文与完整图片 | 仅解析 QQ 中的帖子 JSON 分享卡片 |
...
腾讯频道仅解析 QQ/OneBot 上报的帖子 JSON 分享卡片，并使用卡片中的公开帖子标识读取正文、完整图片列表和帖子视频。直接发送 `pd.qq.com` 链接不会触发解析，评论暂不解析；插件不会保存卡片令牌、匿名访客标识或 QQ 登录 Cookie。详情接口不可用时会降级展示卡片标题和封面。
